...
class PositionEncoder(nn.Module):
    ...
    def define_parameter(self):
        ...
        elif self.posenc_type == 'cpb':
            self.cpb_mlp = nn.Sequential(
                nn.Linear(2, self.hidden_dims, bias=True),
                nn.ReLU(inplace=True),
                nn.Linear(self.hidden_dims, self.hidden_dims, bias=True))
            self.proj = nn.Linear(self.enc_dims, self.head)
        elif self.posenc_type == 'cpb1111':
            self.cpb_mlp = nn.Sequential(
                nn.Linear(4, self.hidden_dims, bias=True),
                nn.ReLU(inplace=True),
                nn.Linear(self.hidden_dims, self.hidden_dims, bias=True))
            self.proj = nn.Linear(self.enc_dims, self.head)
    ...
    def forward(self, positions, cells=None):
        ...
        if self.posenc_type == 'sinusoid' or self.posenc_type == 'ipe' or self.posenc_type == 'sinusoid2':
            # self.b_vals = self.b_vals.cuda()
            # self.a_vals = self.a_vals.cuda()
            self.b_vals = self.b_vals.to(device=device)
            self.a_vals = self.a_vals.to(device=device)
            ...
            pos_enocoding = self.cpb_mlp(positions)
        elif self.posenc_type == 'cpb1111':
            pos_enocoding = self.cpb_mlp(positions)
