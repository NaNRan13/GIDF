...
class LIT_MiddleburyDataset_Noise(Dataset):
    def __init__(self, root='data/Middlebury/', split='train', 
        ...
        if self.split == 'train':
            ...
            self.depth_files = sorted(glob.glob(os.path.join(root, 'gt', '*'))) 
            assert len(self.image_files) == len(self.depth_files)
            self.size = len(self.image_files)
            
        print("========Use noise middlebury dataset=======")
    ...
    def collate_fn(self, datas):
        ...
        for idx, data in enumerate(datas):
            ...
            if self.noisy:
                ...
                depth_lr = add_noise(depth_lr, sigma=651)
