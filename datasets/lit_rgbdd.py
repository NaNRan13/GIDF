...
class LIT_RGBDDDataset(Dataset):
    def __init__(self, root='/data/Lu/', split='train', 
        ...
        # rgbdd = True is Test_Realscene, rgbdd = False is large dataset
        self.rgbdd = False
        ...
        if self.split == 'train':
            raise AttributeError('RGBDD dataset only support test mode.')
        else:
            self.image_files = sorted(glob.glob(os.path.join(root, 'RGB', '*'))) # the name escape a `t`...
            self.depth_files = sorted(glob.glob(os.path.join(root, 'Depth', '*')))
            if self.rgbdd:
                print("================Use RGBDD_Test_Realscene dataset================")
                self.depth_lr_files = sorted(glob.glob(os.path.join(root, 'DepthLR', '*')))
            else:
                print("================Use RGBDD dataset================")
    ...
    def __getitem__(self, idx):
        ...
        image = skimage.io.imread(image_file).astype(np.uint8) # [H, W, 3]
        depth_hr = skimage.io.imread(depth_file).astype(np.float32) / 1000# [H, W]
        
        max = depth_hr.max()
        min = depth_hr.min()
        
        if self.rgbdd:
            depth_lr_files = self.depth_lr_files[idx]  
            depth_lr = cv2.imread(depth_lr_files)[:,:,0].astype(np.float32) / 1000 # [H, W]
