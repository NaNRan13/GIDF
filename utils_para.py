...
class RMSEMeter:
    ...
    def update(self, data, preds, truths, csv_writer=None, eval=False):
        ...
        if eval:
            ...
            # 为什么使用hr_depth的尺寸
            # 目前观察到的差别在于, hr_depth的通道为1, hr_image的通道为3
            B, C, H, W = data['hr_depth'].shape
...
class Trainer(object):
    def __init__(self, 
                 ...
                 max_keep_ckpt=1000, # max num of saved ckpts in disk
        ...
        self.metrics_de = RMSEMeter(self.args)
        self.metrics_c = RMSEMeter(self.args)
        
        self.model.to(self.device)
        

        n_gpus = len(os.environ['CUDA_VISIBLE_DEVICES'].split(','))
        if n_gpus > 1 and args.para:
            if self.args.para_block == 0:
                raise NotImplementedError(f'did not set para_block')
            self.model.to('cuda:1')
            if args.num_bif <= 2:
                raise NotImplementedError(f'this config can run without model para') 
            self.model.depth_conv.to('cuda:0')
            self.model.rgb_conv.to('cuda:0')
            self.model.patch_embed_depth.to('cuda:0')
            self.model.patch_embed_rgb.to('cuda:0')
            self.model.pos_drop.to('cuda:0')
            self.model.guidedlayer[0].to('cuda:0')
            self.model.depthlayer[0].to('cuda:0')
            for i_layer in range(0, self.args.para_block):
                self.model.bif[i_layer].to('cuda:0')
                self.model.guidedlayer[i_layer+1].to('cuda:0')
                self.model.depthlayer[i_layer+1].to('cuda:0')
        elif n_gpus > 1 and not args.para:
            self.model = nn.parallel.DataParallel(self.model, device_ids=range(n_gpus))
        elif n_gpus == 1 and args.para:
            raise NotImplementedError(f'model para is true, but got 1 gpu')
        ...
        # workspace prepare
        self.workspace = f"{self.workspace}/{self.name}"
        self.log_ptr = None
        if self.workspace is not None:
            os.makedirs(self.workspace, exist_ok=True)        
            self.log_path = os.path.join(self.workspace, f"log_{self.name}.txt")
            self.config_path = os.path.join(self.workspace, "config.txt")
        ...
        if not self.args.test:
            self.log('[INFO] Model struct={}'.format(model))
        ...
        if self.workspace is not None:
            if self.use_checkpoint == "scratch":
                ...
                if os.path.exists(the_path):
                    self.log("[INFO] Loading best checkpoint ...")
                    self.log(f"[INFO] Loading {the_path}")
    ...
    def train_step(self, data):
        ...
        # gt = data['hr_depth']
        gt = data['hr_pixel']
        B, C, H_gt, W_gt = data['hr_depth'].shape
        B, C, H, W = data['hr_image'].shape
        shape = [B, H, W, 1]
        ...
        # a = time.time()
        # l1 + l2 
        # depth_sr, cdepth_sr, fudepth_sr  = self.model(data)
        # just l2 
        depth_sr, fudepth_sr  = self.model(data)
        
        loss = self.objective(fudepth_sr, gt)
        ...
        # depth_sr = depth_sr * (data['max'] - data['min']) + data['min']
        # cdepth_sr = cdepth_sr * (data['max'] - data['min']) + data['min']
        # fudepth_sr = fudepth_sr * (data['max'] - data['min']) + data['min']
        # gt = gt * (data['max'] - data['min']) + data['min']
        
        return depth_sr, fudepth_sr, gt, loss

    def eval_step(self, data):
        B, C, H, W = data['hr_depth'].shape
        gt = data['hr_depth']
        input = data["lr_depth"]
        coord_h, coord_w = data['hr_coord'].shape[1:3]
        shape = [B, coord_h, coord_w, 1]

        depth_sr, fudepth_sr  = self.model(data, eval_mode=True)

        depth_sr = depth_sr.view(*shape).permute(0, 3, 1, 2)
        fudepth_sr = fudepth_sr.view(*shape).permute(0, 3, 1, 2)
    
        depth_sr = depth_sr[...,:H, :W]
        fudepth_sr = fudepth_sr[...,:H, :W]
    ...
    def test_step(self, data):  
        ...
        # print(fudepth_sr.shape, gt.shape)
        ...
        return depth_sr, fudepth_sr, gt, dtime, input

    ### ------------------------------

    def train(self, train_loader_fix=None, train_loader=None, valid_loader=None, max_epochs=200, AR_epoch = None):
        ...
        for epoch in range(self.epoch, max_epochs + 1):
            ...
            if AR_epoch >= epoch and self.two_stage:
                #训练
                ...
                #验证
                if self.epoch < AR_epoch-50:
                    if self.epoch % 10 == 0:
                        self.evaluate_one_epoch(valid_loader)
                        self.save_checkpoint_fix(full=True, best=True)
                else:
    ...
    def test(self, loader, save_path=None):
        if save_path is None:
            save_path_d = os.path.join(self.workspace, 'results_d', f'{self.name}_{self.args.dataset}_{self.args.scale}')
            save_path_fu = os.path.join(self.workspace, 'results_fu', f'{self.name}_{self.args.dataset}_{self.args.scale}')
            csv_path = os.path.join(self.workspace, f'{self.name}_{self.args.dataset}_{self.args.scale}.csv')
        os.makedirs(save_path_d, exist_ok=True)
        os.makedirs(save_path_fu, exist_ok=True)
        f = open(csv_path, 'w', encoding='utf-8')
        csv_writer  = csv.writer(f)
        
        print(" this is a test ")
        self.log(f"==> Start Test, save results to {save_path}")
        self.log(f"==> Test dataset:", self.args.dataset, "Scale:", self.args.scale, "<==")
        ...
        with torch.no_grad():
            ...
            for data in loader:
                ...
                depth_sr, fudepth_sr, gt, dtime, input = self.test_step(data)
                # depth_sr, cdepth_sr, fudepth_sr, gt, loss = self.eval_step(data) 
                # all_time += dtime
                for metric in self.metrics:
                    if self.args.dataset == 'NYU':
                        metric.update(data, fudepth_sr, gt, csv_writer, eval=True)
                    else:
                        metric.update(data, fudepth_sr, gt, csv_writer, eval=False)

                # preds = preds.detach().cpu().numpy() # [B, 1, H, W]
                depth_sr = depth_sr.detach().cpu().numpy()
                fudepth_sr = fudepth_sr.detach().cpu().numpy()
                gt = gt.detach().cpu().numpy() # [B, 1, H, W]
                # input = input.detach().cpu().numpy()

                for b in range(fudepth_sr.shape[0]):
                    idx = data['idx'][b]
                    if not isinstance(idx, str):
                        idx = str(idx.item())
                    # pred = preds[b][0]
                    depth_sr = depth_sr[b][0]
                    fudepth_sr = fudepth_sr[b][0]
                    gt = gt[b][0]
                    # input = input[b][0]
                    # error = abs(gt-fudepth_sr)
                    # error = np.sqrt(error)
                    # aa = np.min(gt)
                    # bb = np.max(gt)
                    ...
                    plt.imsave(os.path.join(save_path_d, self.name+idx+'.png'),depth_sr, cmap="gray")
                    plt.imsave(os.path.join(save_path_fu, self.name+idx+'.png'),fudepth_sr, cmap="gray")

                    # cv2.imwrite(os.path.join(save_path_d, self.name + idx + ".png"),depth_sr)
                    # cv2.imwrite(os.path.join(save_path_c, self.name + idx + ".png"),cdepth_sr)
                    # cv2.imwrite(os.path.join(save_path_fu, self.name + idx + ".png"),fudepth_sr)
    ...
    def train_one_epoch(self, loader):
        ...
        for data in loader:
            ...
            data = self.prepare_data(data)
            # 前向并计算loss
            depth_sr, fudepth_sr, truths, loss = self.train_step(data)
            # loss *= 100
            ...
            if self.local_rank == 0:
                for metric in self.metrics:
                    metric.update(data, fudepth_sr, truths)
        ...
        self.log(f'time={(end_time-start_time)/60:.2f} min')
        self.log_ptr.flush()
        torch.cuda.empty_cache()


    def evaluate_one_epoch(self, loader, save_path=None):
        self.log(f"<------- {self.epoch} -------> Evaluate at epoch <------- {self.epoch} ------->")
        self.sum_t = 0
        ...
        if self.local_rank == 0:
            ...
            # self.metrics_c.clear()
            self.metrics_de.clear()
        ...
        if save_path is None:
            save_path_d = os.path.join(self.workspace, 'results_d', f'{self.name}_{self.args.dataset}_{self.args.scale}')
            # save_path_c = os.path.join(self.workspace, 'results_c', f'{self.name}_{self.args.dataset}_{self.args.scale}')
            save_path_fu = os.path.join(self.workspace, 'results_fu', f'{self.name}_{self.args.dataset}_{self.args.scale}')
        ...
        with torch.no_grad():
            ...
            for data in loader:    
                ...
                # a = time.time()
                depth_sr, fudepth_sr, truths, loss = self.eval_step(data)   #truths is gt
                # b = time.time()
                # self.sum_t += b-a
                ...
                if self.local_rank == 0:
                    for metric in self.metrics:
                        metric.update(data, fudepth_sr, truths, eval=True)
                    pbar.set_description(f'loss={total_loss[-1]:.4f}')
                    pbar.update(loader.batch_size * self.world_size)

                    self.metrics_de.update(data, depth_sr, truths, eval=True)
                    # self.metrics_c.update(data, cdepth_sr, truths, eval=True)
        ...
        if self.local_rank == 0:
            ...
            # self.log(self.metrics_de.report()) 
            self.log(f"depth_de RMSE = {self.metrics_de.measure():.6f}")
            # self.log(f"depth_c RMSE = {self.metrics_c.measure():.6f}")
            self.metrics_de.clear()
            # self.metrics_c.clear()
        
        depth_sr = depth_sr.detach().cpu().numpy()
        # cdepth_sr = cdepth_sr.detach().cpu().numpy()
        fudepth_sr = fudepth_sr.detach().cpu().numpy()
        gt = truths.detach().cpu().numpy() # [B, 1, H, W]
        # input = input.detach().cpu().numpy()
        ...
        for b in range(fudepth_sr.shape[0]):
            ...
            # plt.imsave(os.path.join(error_path,  self.name+'error'+idx+'.png'), error,vmin=5,vmax=50,  cmap="plasma")
            plt.imsave(os.path.join(save_path_d, self.name+idx+'.png'),depth_sr, cmap="gray")
            # plt.imsave(os.path.join(save_path_c, self.name+idx+'.png'),cdepth_sr, cmap="gray")
            plt.imsave(os.path.join(save_path_fu, self.name+idx+'.png'),fudepth_sr, cmap="gray")
    ...
    def load_checkpoint(self, checkpoint=None):
        ...
        if self.use_checkpoint == 'from':
            ...
            # print(self.epoch)
            # if self.epoch >= 375:
            #     checkpoint_dict['lr_scheduler']['step_size'] = 25 
            try:
                self.stats["best_result"] = checkpoint_dict['best_result']
                self.log("[INFO] loaded best_result.")
            except:
                self.log("[WARN] Failed to load best_result. Skipped.")
            try:
        ...
        if 'amp' in checkpoint_dict:
            try:
                ...
                self.log("[WARN] Failed to load amp. Skipped.")